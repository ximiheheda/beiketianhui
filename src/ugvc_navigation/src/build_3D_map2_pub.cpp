...
class build_3D_map
{
  ...
  ros::Publisher flight_command_pub;
...
{
  points_sub = nh.subscribe <sensor_msgs::PointCloud2> ("/velodyne_points", 1, &build_3D_map::cloud_cb,this);
  ...
  flight_command_pub= nh.advertise<ugvc_navigation::flight_command> ("/flightcommand",10);
  action_command_pub= nh.advertise<std_msgs::Int8>("actioncommand",1);
  last_wall_distance=100;
  num_window_passed=0;  
  while(ros::ok())
  {
    ros::Rate rate(10.0); 
...
void build_3D_map::cloud_cb (const sensor_msgs::PointCloud2ConstPtr& input) 
{
  ...
  try{
    //  ROS_INFO("SSS");
      plistener.waitForTransform("/odom", "/velodyne",
                                  ros::Time::now(), ros::Duration(10));
      plistener.lookupTransform("/odom", "/velodyne",
  ...
  if(abs(uav_location.point.x)<5&&abs(uav_location.point.y)<5&&abs(uav_location.point.z)<0.8)
  {
  ...
  for(size_t i=0;i<cloud.size();i++)
  {
    ...
    if(abs(base_point.point.x)<5&&abs(base_point.point.y)<5&&abs(base_point.point.z)<0.8)//&&vv>=0&&uu>=0&&ww>=0)
    {
  ...
  for(i=uu_uav;i<uu_uav+30;i++)//只扫描前方三米
  {
    ...
    if(num_obstacle >20) //若是墙，则该行不少于20
    {
  ...
  for(size_t j=0; j<globalMapLongth; j++)
  {
      ...
      if(mazemap.maparry[(uu_wall_start-3)+j*globalMapWidth+ww_uav*globalMapLongth*globalMapLongth] == 1)
      {
        if(uu_wall_min <= uu_wall_start - 3) uu_wall_min = uu_wall_start-3;
      }
  ...
  {
    break_start_index = 0; 
    for(size_t vv_temp=0; vv_temp<globalMapLongth-20; vv_temp++) //vv_temp设置为了防止数组越界
    {
  ...
  current_wall_distance=uu_wall_min-50;
  flight_command_msg.front_wall_distance=current_wall_distance;
  if(current_wall_distance-last_wall_distance>5);
  {
    num_window_passed++;
  }
  ROS_INFO("current wall %d, last wall %d",int(current_wall_distance),int(last_wall_distance));
  flight_command_msg.num_window_passed=num_window_passed;
  last_wall_distance=current_wall_distance;

  //wall reach or not
  if(current_wall_distance<19 && current_wall_distance>=15)
  {
    flight_command_msg.front_wall_reach=true;  
    flight_command_msg.front_wall_too_near=false;
  }
  else if(current_wall_distance<15 && current_wall_distance>0)
  {
    flight_command_msg.front_wall_reach=false;
    flight_command_msg.front_wall_too_near=true;
  }
  else
  {
    flight_command_msg.front_wall_reach=false;
  ...
  //左右
  if(window_average_vv>50)
  {
    
    flight_command_msg.window_is_right=true;
    flight_command_msg.window_is_left=false;
  }
  else if (window_average_vv<50 && window_average_vv>0)
  {
    flight_command_msg.window_is_right=false;
    flight_command_msg.window_is_left=true;
  }
  else
  {
    flight_command_msg.window_is_right=false;
  ...
  if(obstacle_front_map.maparry[uu_wall_min+vv_uav*globalMapWidth+ww_uav*globalMapLongth*globalMapLongth]==0 && 
    obstacle_front_map.maparry[uu_wall_min+(vv_uav-1)*globalMapWidth+ww_uav*globalMapLongth*globalMapLongth]==0 &&
    obstacle_front_map.maparry[uu_wall_min+(vv_uav+1)*globalMapWidth+ww_uav*globalMapLongth*globalMapLongth]==0 &&
    obstacle_front_map.maparry[uu_wall_min+(vv_uav-2)*globalMapWidth+ww_uav*globalMapLongth*globalMapLongth]==0 && 
    obstacle_front_map.maparry[uu_wall_min+(vv_uav+2)*globalMapWidth+ww_uav*globalMapLongth*globalMapLongth]==0 &&
  ...
  {
    flight_command_msg.safe=false;
  }
  
  flight_command_pub.publish(flight_command_msg);

  //action_command
  action_command_msgs.data=FORWARD;//默认向前
  if(flight_command_msg.safe )
  {
    action_command_msgs.data=FORWARD;
  }
  else
  {
    if(obstacle_front_map.maparry[uu_wall_min+(vv_uav-1)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth]==0 &&
      obstacle_front_map.maparry[uu_wall_min+(vv_uav+1)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth]==0 &&
      ...
      action_command_msgs.data=FORWARD;
    else if(flight_command_msg.front_wall_reach)
    {
